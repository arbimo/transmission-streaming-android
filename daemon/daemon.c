 ...
 *
 * $Id: daemon.c 10659 2010-05-18 22:25:14Z Longinus00 $
 */
...
static const struct tr_option options[] =
{
    ...
    { 854, "zipf", "Use the zipf scheduling policy", NULL, 1, "<teta>" },
    ...
    while(( c = tr_getopt( getUsage(), argc, (const char**)argv, options, &optarg ))) {
        switch( c ) {
            ...
            case 854: tr_bencDictAddBool( &settings, TR_PREFS_KEY_USE_ZIPF, TRUE );
                      tr_bencDictAddReal( &settings, TR_PREFS_KEY_ZIPF_TETA, atof( optarg ) );
                      break;
